            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">
                        <a href="{{ route('our-departments.index') }}">Departments 
                            - <small style="color: #eb252a;">count( {{ $departments->count() }} )</small>
                        </a>
                    </li>
                </ol>
            </nav>
                            ...
                            <td>
                                @if($department->images)
                                    @foreach(json_decode($department->images, true) as $img)
                                        <img src="{{ asset('images/departments/' . $img) }}" class="img-resposive mb-1" width="100px" height="50px">
                                    @endforeach
                                @endif
                            </td>
                            <td style="display: flex; gap:5px;">
                                <a href="{{ route('our-departments.edit',$department->id) }}">
                                    <button class="btn btn-success" type="button"><i class="fa fa-pencil"></i></button>
                                </a>
                                <form action="{{ route('our-departments.destroy',$department->id) }}" method="post">
                                    {{ method_field('delete') }}
                                    @csrf
...
function updateOrder(data) {
    ...
    var ajaxurl = '{{ route("sort_department") }}'; // <-- create this route in web.php
