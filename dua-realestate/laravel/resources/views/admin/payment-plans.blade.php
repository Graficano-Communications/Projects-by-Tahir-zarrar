        ...
        <!-- table section -->
            <div class="white_shd full margin_bottom_30">
               <div class="full graph_head d-flex justify-content-between">
                  ...
                  <div>
                    <a href="{{ url('add-payment-plan') }}" type="button" class="btn btn-info">
                        ...
                        <tbody>
                            @foreach($paymentPlans as $plan)
                                <tr>
                                    ...
                                    <td>
                                        <img src="{{ asset('uploads/payment_plans/' . $plan->plan_image) }}" alt="{{ $plan->plan_name }}" style="width: 100px;">
                                    </td>
                                    <td>{{ $plan->project->name ?? 'N/A' }}</td>
                                    <td>{{ $plan->protype->type_description ?? 'N/A' }}</td>
                                    {{-- <td>{{ $plan->sequence }}</td> --}}
                                    <td>
                                        <!-- Add edit and delete actions here -->
                                        <a href="{{ route('payment-plans.edit', $plan->id) }}" class="btn btn-success">Edit</a>
                                        <form action="{{ route('payment-plans.destroy', $plan->id) }}" method="POST" style="display:inline;">
                                            @csrf
                                            @method('DELETE')
