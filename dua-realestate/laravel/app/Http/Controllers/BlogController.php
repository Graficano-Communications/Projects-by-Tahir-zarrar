...
class BlogController extends Controller
{
    ...
    public function update_blog(Request $request, $id)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            if (!empty($blog->image) && File::exists(public_path('uploads/blogs/' . $blog->image))) {
            ...
            $imageFile->move(public_path('uploads/blogs/'), $imageFilename);
        ...
        if ($request->hasFile('banner_image')) {
            ...
            if (!empty($blog->banner_image) && File::exists(public_path('uploads/blogs/' . $blog->banner_image))) {
                File::delete(public_path('uploads/blogs/' . $blog->banner_image));
            }
    
            // Upload new banner image
            $bannerFile = $request->file('banner_image');
            $bannerExtension = $bannerFile->getClientOriginalExtension();
            $bannerFilename = time() . '_banner.' . $bannerExtension;
            $bannerFile->move(public_path('uploads/blogs/'), $bannerFilename);
    
            // Update banner image field in the database
            $blog->banner_image = $bannerFilename;
        }
