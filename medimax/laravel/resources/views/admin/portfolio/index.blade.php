    ...
    <div class="row justify-content-center">
        ...
        <div class="col-md-10">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active"><a href="{{ route('teams') }}">Portfolios
                            - <small style="color: #eb252a;">count( {{ $portfolio->count() }} )</small></a></li>
                </ol>
            </nav>
        </div>
        <div class="col-md-2">
        ...
        <table class="table table-striped bg-white">
            ...
            <tbody id="BannersSortable">
                @foreach ($portfolio as $key => $testimonial)
                <tr id="{{ $testimonial->id }}">
                    ...
                    <td>
                        ...
                        <form action="{{ route('portfolio.approve', $testimonial->id) }}" method="post">
                            {{ method_field('post') }}
                            @csrf
                            <select class="form-select {{ $testimonial->status === 'private' ? 'bg-secondary text-white' : '' }}" name="status" onchange="this.form.submit()">
                    ...
                    <td>
                        <img src="{{ asset($testimonial->front_image) }}" alt="..." class="avatar shadow">
                    </td>
                    ...
                    <td>
                        <div style="display: flex;gap:5px;">
                            ...
                            <a href="{{ route('portfolio.edit', $testimonial->id) }}"><button class="btn btn-success" type="button"><i class="fa fa-pencil"></i></button></a>
                            <form action="{{ route('portfolio.destroy', $testimonial->id) }}" method="post">
                                {{ method_field('delete') }}
                                @csrf
...
<script>
    ...
    function updateOrder(data) {
        ...
        var ajaxurl = "{{ route('sort_portfolio') }}";
        var data = {
            'data': data,
            '_token': token
        };
